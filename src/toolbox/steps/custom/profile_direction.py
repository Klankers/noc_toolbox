...
class BlankStep(BaseStep, QCHandlingMixin):
    ...
    def run(self):
        ...
        data_subset = self.data[["PROFILE_NUMBER", "PRES", "TIME"]].where(
            ~nan_mask, drop=True
        )

        # Find the gradient of pressure over time
        data_subset = data_subset.set_coords(["TIME"])
        data_subset["direction"] = -1 * np.sign(
            data_subset["PRES"].differentiate("TIME", datetime_unit="s")
        )

        # Find the median direction per profile
        direction_mapping = data_subset.groupby("PROFILE_NUMBER").map(
            lambda x: x["direction"].median()
        )
        ...
        self.data["PROFILE_DIRECTION"] = xr.DataArray(
            np.full(len(self.data["N_MEASUREMENTS"]), np.nan), dims=["N_MEASUREMENTS"]
        )
        ...
        self.update_qc()
        self.generate_qc(
            {"PROFILE_DIRECTION_QC": ["PROFILE_NUMBER_QC", "PRES_QC", "TIME_QC"]}
        )
